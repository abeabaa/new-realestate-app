...
def load_data(file_path):
    ...
    try:
        # 🚨 시트 이름을 "1.매매증감", "2.전세증감"으로 수정했습니다.
    ...
    rent = rent.dropna(subset=['구분']) # rent 데이터에도 dropna 적용
...
selected_regions = st.sidebar.multiselect(
    "지역 선택",
    options=all_regions,
    default=list(all_regions[:5]) # 기본값: 처음 5개 지역 선택
)
...
if df_sel.empty:
    ...
    fig = px.line(
        ...
        color_discrete_map=color_map # 사용자가 선택한 색상 맵을 적용
    )
